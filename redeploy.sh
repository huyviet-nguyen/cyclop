...
kubectl exec -it "$kafka_pod" -- sh -c '/usr/bin/kafka-topics --create --if-not-exists --bootstrap-server kafka:9092 --replication-factor 1 --partitions 40 --topic kline --config retention.ms=10000 && /usr/bin/kafka-topics --describe --bootstrap-server kafka:9092 --topic kline '
sleep 10;
cd .. ;
cd apps;
